 ...
 *
 * Usage:
 *   node scripts/fetch-questions.js --token "MLY|xxx" --count 100 --output questions.json
 */

// ─── 全球城市/区域 bbox 列表 (west, south, east, north) ─────────────────────
const BBOXES = [
  // ── 东亚 ──
...
function parseArgs() {
  ...
  const opts = { count: 100, output: "questions.json" };
  ...
  if (!opts.token) {
    ...
    console.error("Usage: node fetch-questions.js --token \"MLY|xxx\" [--count 100] [--output questions.json]");
    process.exit(1);
  }
...
/** Haversine 公式：计算两个经纬度之间的球面距离（km） */
...
/** 检查候选图片是否与已收集的图片距离过近 */
function isTooClose(candidate, collected, minDistKm) {
...
// ─── Mapillary API 调用 ──────────────────────────────────────────────────────
async function fetchMapFeatures(token, bbox, limit = PAGE_LIMIT, after = null) {
  const [w, s, e, n] = bbox;
  ...
  if (!res.ok) {
    ...
    throw new Error(`Mapillary API ${res.status}: ${body}`);
  }
...
// ─── 主逻辑 ──────────────────────────────────────────────────────────────────
async function main() {
  const { token, count, output } = parseArgs();

  console.log(`目标题目数量: ${count}`);
  console.log(`输出文件: ${output}`);
  console.log(`可用区域数: ${BBOXES.length}`);
  console.log();

  const collected = new Map(); // imageId -> { imageId, latitude, longitude }

  // 每个区域维护独立的分页游标，避免重复抓取同一批图片
  const cursors = new Map(); // regionName -> after cursor

  // 每轮从每个区域只取 1 页（100 张），多轮交替，保证全球均匀分布
  const MAX_ROUNDS = 10;
  // 同一区域内两张图片的最小间距（km），避免同一条路上重复采样
  const MIN_DISTANCE_KM = 5;

  for (let round = 0; round < MAX_ROUNDS && collected.size < count; round++) {
    // 每轮打乱区域顺序，避免固定的城市采集顺序
    const regions = shuffle(BBOXES);
    console.log(`\n── 第 ${round + 1} 轮 (已收集 ${collected.size}/${count}) ──`);

    for (const region of regions) {
      if (collected.size >= count) break;

      const cursorKey = region.name;
      const after = cursors.get(cursorKey) ?? null;

      console.log(`  [${collected.size}/${count}] ${region.name}`);

      try {
        // 每次只取 1 页（最多 100 张），然后跳到下一个区域
        const results = [];
        const seen = new Set();
        const json = await fetchMapFeatures(token, region.bbox, PAGE_LIMIT, after);
        const features = json.data || [];

        for (const feat of features) {
          const images = feat.images?.data || [];
          for (const img of images) {
            if (img.id && img.geometry?.coordinates && !seen.has(String(img.id))) {
              const [lon, lat] = img.geometry.coordinates;
              seen.add(String(img.id));
              results.push({ imageId: String(img.id), latitude: lat, longitude: lon });
            }
          }
        }
        ...
        if (paging?.next) {
          try {
            ...
            if (nextAfter) cursors.set(cursorKey, nextAfter);
          } catch { /* ignore */ }
        }
        ...
        for (const img of shuffled) {
          ...
          if (isTooClose(img, collected, MIN_DISTANCE_KM)) continue;
          collected.set(img.imageId, img);
          added++;
        }

        if (added > 0) {
          console.log(`    -> +${added} (本区域累计 ${collected.size} 去重后)`);
        }
  ...
  const result = shuffle([...collected.values()]);

  if (result.length === 0) {
    console.error("\n未能获取到任何图片。请检查 token 是否有效。");
    process.exit(1);
  }
  ...
  console.log(`\n完成! 共 ${result.length} 道题目已写入 ${output}`);
  if (result.length < count) {
    console.log(`(请求 ${count} 道，实际可用 ${result.length} 道)`);
  }
}
